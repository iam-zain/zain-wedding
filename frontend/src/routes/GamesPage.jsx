...
const GAMES = [
  { id: 'arrange', emoji: '📜', title: 'Arrange the Nikah', hint: 'Rasmein sahi tartib mein lagao', color: '#25d366', Comp: ArrangeNikah,
    best: (s) => (s.best.arrange != null ? `Best ${s.best.arrange}/8` : null) },
  { id: 'tap', emoji: '🎯', title: 'Baraat Reflex', hint: 'Uchhalti cheezon mein sahi wali dabao', color: '#0095f6', Comp: TapCorrect,
  ...
  { id: 'bouquet', emoji: '💐', title: 'Build the Bouquet', hint: 'Order pe bouquet banao — ya apna', color: '#f472b6', Comp: BuildBouquet,
    best: (s) => (s.best.bouquet ? `Best ${s.best.bouquet} bouquet` : null) },
  { id: 'hidden', emoji: '👀', title: 'Chhupa Dil', hint: 'Bheed mein chhupa dil dhoondo — sirf 5 second', color: '#a855f7', Comp: HiddenHeart,
    best: (s) => (s.hidden ? `${s.hidden} dil mile · fastest ${s.best.hidden}s` : null) },
  { id: 'ttt', emoji: '🤵', title: 'Choose Yours', hint: 'Zain 🤵 vs Uzma 👰 — shaadi wala tic-tac-toe', color: '#0095f6', Comp: ChooseYours,
    best: () => null },
  { id: 'crj', emoji: '🚂', title: 'CRJ Wedding Express', hint: 'Switch badlo, train ko shaadi tak pahunchao', color: '#f7971e', Comp: CrjExpress,
    best: (s) => (s.best.crj ? `Best ${s.best.crj} station` : null) },
  { id: 'shoot', emoji: '💘', title: "Cupid's Arrow", hint: 'Dil pe nishana — ☠️ se bacho!', color: '#ed4956', Comp: ShootHearts,
    best: (s) => (s.best.shoot != null ? `Best ${s.best.shoot}` : null) },
  { id: 'catch', emoji: '💘', title: 'Dil Pakdo', hint: 'Dil pakdo — 20 second mein 10 points', color: '#ff6b81', Comp: CatchHearts,
    best: (s) => (s.best.catch != null ? `Best ${s.best.catch} · ${s.hearts} dil pakde` : null) },
  { id: 'hold', emoji: '🫶', title: 'Dil Sambhalo', hint: 'Tokri se girte dil pakdo', color: '#a855f7', Comp: HoldHeart,
    best: (s) => (s.best.hold != null ? `Best ${s.best.hold}` : null) },
  { id: 'puzzle', emoji: '🥰', title: 'Emoji Shaadi Puzzle', hint: 'Emoji padho, shaadi ki rasm pehchano', color: '#f7971e', Comp: EmojiPuzzle,
    best: (s) => (s.puzzles.length ? `${s.puzzles.length}/${PUZZLES.length} puzzles hal kiye` : null) },
  { id: 'connect', emoji: '💞', title: 'Heart Connect', hint: 'Ek jaise dil jodo, lines na kaatein', color: '#ed4956', Comp: HeartConnect,
    best: (s) => (s.best.connect ? `Level ${s.best.connect} tak` : null) },
  { id: 'memory', emoji: '💗', title: 'Jodi Milao', hint: 'Jode milao — bina galti ke', color: '#25d366', Comp: HeartMemory,
    best: (s) => (s.best.memory != null ? `Best ${s.best.memory} chaal` : null) },
  { id: 'scramble', emoji: '💌', title: 'Love Letter Scramble', hint: 'Bikhre akshar — MUHABBAT banao', color: '#f472b6', Comp: LetterScramble,
  ...
  { id: 'nikahpuzzle', emoji: '🧩', title: 'Nikah Puzzle', hint: 'Shaadi ki tasveer jodo — 3×3 ya 4×4', color: '#a855f7', Comp: NikahPuzzle,
    best: (s) => (s.best.puzzle3 != null ? `3×3 best ${s.best.puzzle3}s` : null) },
  { id: 'pack', emoji: '🧳', title: 'Pack for the Waleema', hint: 'Belt se sahi saamaan uthao', color: '#0095f6', Comp: PackWaleema,
  ...
  { id: 'ringbox', emoji: '🎁', title: 'Ring in the Box', hint: 'Dabbe ghoomenge — ring wala pakdo', color: '#f7971e', Comp: RingBox,
    best: (s) => (s.best.ringbox ? `Best ${s.best.ringbox} round` : null) },
  { id: 'ringstack', emoji: '💍', title: 'Ring Stack', hint: 'Rings ko sahi order mein lagao', color: '#f5c518', Comp: RingStack,
    best: (s) => (s.best.ringstack ? `Best level ${s.best.ringstack}` : null) },
  { id: 'trueheart', emoji: '😳', title: "Sachha Dil", hint: 'Sirf sachha dil ❤️ dabao — 💔 se bacho', color: '#ed4956', Comp: TrueHeart,
    best: (s) => (s.best.trueheart != null ? `Best ${s.best.trueheart} rounds · kul ${s.trueHeart}` : null) },
  { id: 'gift', emoji: '🎁', title: 'Tohfa Rush', hint: 'Tohfe sahi dabbe mein daalo', color: '#f7971e', Comp: GiftRush,
  ...
  { id: 'maze', emoji: '👰', title: 'Wedding Maze', hint: 'Ungli se rasta banao — dulhan ko dulhe tak', color: '#d4a64a', Comp: WeddingMaze,
    best: (s) => (s.best.maze ? `Best ${s.best.maze} maze paar` : null) },
  { id: 'spotter', emoji: '📸', title: 'Wedding Spotter', hint: 'Scene mein 7 cheezein dhoondo', color: '#f7971e', Comp: WeddingSpotter,
